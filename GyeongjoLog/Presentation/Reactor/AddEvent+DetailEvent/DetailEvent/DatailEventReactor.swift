...
class DatailEventReactor: ReactorKit.Reactor, Stepper {
    ...
    init(eventUseCase: EventUseCase, addEventFlow: AddEventFlow, event: EventSummary) {
        ...
        if event.amount == 0 {
            ...
            let formattedAmountText = (isOthersEvent ? "- " : (isMyEvent ? "+ " : "")) + event.amount.formattedWithComma()
        ...
        self.initialState = State(id: event.id, name: event.name, phoneNumber: event.phoneNumber, eventType: event.eventType, date: event.date, relationship: event.relationship, amount: event.amount, formattedAmount: formattedText, eventAmounts: addEventFlow == .myEventSummary ? [10000, 50000, 100000, 500000, 1000000] : [-10000, -50000, -100000, -500000, -1000000])
    }
    ...
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .addEventButtonTapped:
            ...
            return self.eventUseCase.saveEvent(event: event)
                .andThen(Completable.create { completable in
                    self.steps.accept(EventHistoryStep.popViewController)
                    completable(.completed)
                    return Disposables.create()
