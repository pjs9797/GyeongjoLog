...
struct EventResponseDTO: Codable {
    ...
    static func toEvent(dto: EventResponseDTO) -> Event {
        let data = dto.data
        return Event(id: String(data.id), name: data.name, phoneNumber: data.phoneNumber, eventType: data.eventType, date: data.date, relationship: data.relationship, amount: data.amount, memo: data.memo)
    }
}
